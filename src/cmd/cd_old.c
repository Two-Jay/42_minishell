...
/*   Updated: 2021/11/27 18:32:55 by jiychoi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	cd_move_directory(t_data *data)
{
	...
	{
		if (chdir(str) < 0)
			return (builtin_error(data, "cd", ft_strjoin(str, CD_ERRNODIR), 1));
	}
	else
	{
		...
		if (return_value < 0)
			return (builtin_error(data, "cd", ft_strjoin(path_absol, CD_ERRNODIR), 1));
	}
	return (0);
}
...
static int	cd_add_oldpwd(t_data *data, t_envlst *node_pwd)
{
	...
	if (!node_oldpwd)
	{
		...
		if (save_env(data, env_key, env_value, ENV) < 0)
			return (-1);
		node_oldpwd = find_env("OLDPWD", data);
...
static int	cd_add_pwd(t_data *data)
{
	...
	if (!node_pwd)
	{
		...
		if (!env_value || save_env(data, env_key, env_value, ENV) < 0)
			return (-1);
		node_pwd = find_env("PWD", data);
	}
...
int	minishell_cd(t_data *data)
{
	...
	if (check_flag(data))
		return (builtin_error(data, "cd",
				ft_strjoin(data->input->next->content, CD_ERROPT), 1));
	if (cd_move_directory(data) < 0)
		return (0);
	return_value = cd_add_pwd(data);
	if (return_value < 0)
		return (builtin_error(data, "cd", ft_strdup(strerror(errno)), 1));
	// $? 세팅: 0
	return (return_value);
}
